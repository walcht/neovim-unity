...
Unity debug adapter:
   ...
   just-installed Unity DA (this is copied from [CGNvim's Unity DAP config][cgnvim_unity_dap]
...
- Q. Neovim keeps getting de-selected in the `Project Settings -> External Tools`
  dropdown menu and another editor is selected again whenver Unity is
  restarted.
- A0. Will be fixed in the near future (this is only occuring on Linux -
  so for some reason, nvim selection is not being persistently saved).
