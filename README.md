...
> [!Warning]
> Depending on the version you used for the analyzer, you may screw up the LSP
> and make its startup time extremely slow (in the range of minutes). I have
> no idea why this is happening but, as mentioned above, simply try to use an
> older version. In case that is still causing issues, move to an even older
> version and so on.
